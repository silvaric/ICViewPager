...
#define kTabOffsetY 0.0
#define kTabWidth 128.0
#define kTabSpacing 0.0
...
@property (nonatomic) NSNumber *tabsSpacing;
...
@synthesize tabsSpacing = _tabsSpacing;
...
- (void)layoutSubviews {
    ...
    CGFloat yOffset = kTabOffsetY;
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        yOffset = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabOffsetY withDefault:yOffset];
    }
    
    CGRect frame = self.tabsView.frame;
    frame.origin.x = 0.0;
    frame.origin.y =  [self.tabLocation boolValue] ? (topLayoutGuide + yOffset) : (CGRectGetHeight(self.view.frame) - [self.tabHeight floatValue] - yOffset);
...
- (void)settabsSpacing:(NSNumber *)tabsSpacing {
    
    if ([tabsSpacing floatValue] < 0.0)
        tabsSpacing = [NSNumber numberWithFloat:kTabSpacing];
    else if ([tabsSpacing floatValue] > CGRectGetWidth(self.view.frame))
        tabsSpacing = [NSNumber numberWithFloat:CGRectGetWidth(self.view.frame)];
    
    _tabsSpacing = tabsSpacing;
}
...
- (void)setActiveContentIndex:(NSUInteger)activeContentIndex {
    ...
    if ([self.delegate viewPager:self valueForOption:ViewPagerOptionCleanOutSightContents withDefault:kCleanOutContents]) {
...
- (NSNumber *)tabsSpacing {
    
    if (!_tabsSpacing) {
        CGFloat value = kTabSpacing;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabsSpacing withDefault:value];
        self.tabsSpacing = [NSNumber numberWithFloat:value];
    }
    return _tabsSpacing;
}
...
- (void)setNeedsReloadOptions {
    ...
    self.tabsSpacing = [NSNumber numberWithFloat:[self.delegate viewPager:self valueForOption:ViewPagerOptionTabsSpacing withDefault:kTabSpacing]];
    ...
    if ([self.tabsSpacing floatValue] != 0.0) {
        contentSizeWidth += (self.tabCount - 1) * [self.tabsSpacing floatValue];
    }
...
- (CGFloat)valueForOption:(ViewPagerOption)option {
    ...
    switch (option) {
        ...
        case ViewPagerOptionTabsSpacing:
            return [[self tabsSpacing] floatValue];
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        else self.tabsView.scrollEnabled = YES;
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        if ([self.dataSource respondsToSelector:@selector(viewPager:widthForTabAtIndex:)]) {
            tabWidthAtIndex = [self.dataSource viewPager:self widthForTabAtIndex:i] + [self.tabsSpacing floatValue];
        }else{
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        if ([self.dataSource respondsToSelector:@selector(viewPager:widthForTabAtIndex:)]) {
            int tabWidthAtIndex = [self.dataSource viewPager:self widthForTabAtIndex:index] + [self.tabsSpacing floatValue];
            tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, tabWidthAtIndex, [self.tabHeight floatValue])];
        }else{
            tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, [self.tabWidth floatValue], [self.tabHeight floatValue])];
        }
        tabView.lineSpacing = [self.tabsSpacing floatValue];
